...
class CombinatorialTestingSetup {
    ...
    // numOfInputs is how many parameters will be tested
    // labels is a list of the parameter names for each input. Leave this blank if it does not matter and you want permutations
    // types is the type of variable each label can take on. If this is only 1 such as 'bool', all inputs can take on true or false.
    constructor(numOfInputs, types) {
        ...
        if(types[0] == 'bool'){
            potentialOptions[0] = false
            potentialOptions[1] = true
        }
        let permutationList = Array()
        this.generateBoolPermutations(numOfInputs, [], potentialOptions, permutationList);
        this.inputCombinations = permutationList;
        // for(let row = 0; row < numOfInputs; row++){
        //     for(let col = 0; col < numOfInputs; col++){
        //         console.log("Bool vals are: " + potentialOptions[row] + " " + potentialOptions[col])
        //     }
        // }
    }

    // Used ChatGPT by asking "How to do boolean permutations generalizable to n in JavaScrip?"
    generateBoolPermutations(n, prefix = [], potentialOptions, permutationList) {
        if (n === 0) {
            //console.log(prefix);
            permutationList.push(prefix);
            return;
        }
        for (const value of potentialOptions) {
            this.generateBoolPermutations(n - 1, [...prefix, value], potentialOptions, permutationList);
        }
    }
...
class GlowCookiesTester {
    constructor() {
      ...
      this.cookiesAllowed = [undefined, undefined, undefined] // Upon saving a selection or selecting accept/ reject these will be updated accordingly
      this.Cookies = undefined
      this.DOMbanner = undefined
    }

    assert(condition, lineNum) {
        if(condition != true){
    ...
    render() {
      ...
      // Tests need to be called after render
      // Call tests as necessary
      //this.test1()
      //this.test2()
      //this.test3()
    ...
    createDOMElements() {
      ...
      this.switch_colors = ["switch_color_off", "switch_color_off", "switch_color_off"]
    ...
    savePreferences(){
      this.Customizer.style.display = "none";
      for(let switch_num = 0; switch_num < 3; switch_num++){
        if(this.switch_on[switch_num] == "glowCookies__customize_switch_button_off"){
    ...
    test1 = () => {
        ...
        this.openManageCookies();
        this.acceptCookies();
        this.assert(localStorage.getItem("GlowCookies") == '1', new Error().lineNumber)
    ...
    test2 = () => {
        ...
        this.config.hideAfterClick = false
        this.openManageCookies();
        this.acceptCookies();
        this.assert(localStorage.getItem("GlowCookies") == '1', new Error().lineNumber)
        this.openManageCookies();
        this.rejectCookies();
        this.assert(localStorage.getItem("GlowCookies") == '0', new Error().lineNumber)
    ...
    ctest1 = () => {
        ...
        this.config.hideAfterClick = false
        let selectorTestInputs = new CombinatorialTestingSetup(3, ['bool'])['inputCombinations'];
        ...
        selectorTestInputs.forEach((selectorTestComb) => {
            ...
            for(let switch_num = 0; switch_num < 3; switch_num++){
                this.assert(this.cookiesAllowed[switch_num] == selectorTestComb[switch_num], new Error().lineNumber);
            }
